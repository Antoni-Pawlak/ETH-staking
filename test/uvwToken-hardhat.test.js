...
contract("UvwToken", (accounts) => {
  ...
  it('Should transfer tokens between accounts', async () => {
    await token.transfer(account1.address, getBNFromNumber(50, dec));
    let account1Balance = await token.balanceOf(account1.address);
    expect(account1Balance).to.equal(getBNFromNumber(50, dec));

    await token.connect(account1).transfer(account2.address, getBNFromNumber(25, dec));
    const account2Balance = await token.balanceOf(account2.address);
    expect(account2Balance).to.equal(getBNFromNumber(25, dec));
  ...
  it('Should failed transfer negative tokens between accounts', async () => {
    console.log("Trying to transfer -10 to account1");
    await expect(token.transfer(account1.address, getBNFromNumber(-10, dec))).to.be.reverted;
    let account1Balance = await token.balanceOf(account1.address);
    expect(account1Balance).to.equal(getBNFromNumber(25, dec));

    await expect(token.connect(account1).transfer(account2.address, getBNFromNumber(-10, dec))).to.be.reverted;
    const account2Balance = await token.balanceOf(account2.address);
    expect(account2Balance).to.equal(getBNFromNumber(25, dec));
  ...
  it('Should failed transfer tokens from account with zero balance', async () => {
    ...
    await expect(token.connect(account3).transfer(account2.address, getBNFromNumber(100, dec))).to.be.reverted;
  ...
  it('Should fail to transfer 2 tokens when balance is 1 tokens in base units', async () => {
    await token.transfer(account5.address, getBNFromNumber(1, dec))
    ...
    await expect(token.connect(account5).transfer(account5.address, getBNFromNumber(2, dec))).to.be.reverted;
    account5Balance = await token.balanceOf(account5.address);
    account6Balance = await token.balanceOf(account6.address);
    expect(account5Balance).to.equal(getBNFromNumber(1, dec));
  ...
  it('Should failed transfer tokens that are greater than current balance', async () => {
    ...
    await expect(token.connect(account1).transfer(account2.address, getBNFromNumber(50, dec))).to.be.reverted;
    account1Balance = await token.balanceOf(account1.address);
    account2Balance = await token.balanceOf(account2.address);
    expect(account1Balance).to.equal(getBNFromNumber(25, dec));
    expect(account2Balance).to.equal(getBNFromNumber(25, dec));
  ...
  it('Account2 burn token with BURNER_ROLE', async () => {
    ...
    await expect(token.connect(account2).burnToken(account2.address, getBNFromNumber(10, dec))).to.be.not.reverted;

    totalSupply = await token.totalSupply();
    account2Balance = await token.balanceOf(account2.address);

    expect(getNumberFromBN(totalSupply, dec)).to.equal(999999990);
    expect(account2Balance).to.equal(getBNFromNumber(15, dec));
  ...
  it('Account1 tries to burn token without BURNER_ROLE, but failed', async () => {
    ...
    await expect(token.connect(account1).burnToken(account1.address, getBNFromNumber(10, dec))).to.be.reverted;
  ...
  it('Should add account2 to blacklist', async () => {
    ...
    await expect(token.connect(account2).transfer(account1.address, getBNFromNumber(10, dec))).to.be.reverted;
  ...
  it('Should remove account2 from blacklist', async () => {
    ...
    await expect(token.connect(account2).transfer(account1.address, getBNFromNumber(10, dec))).to.be.not.reverted;
  ...
  it('Account3 cant transfer before approve', async () => {
    ...
    await expect(token.connect(account3).transferFrom(account2.address, account3.address, getBNFromNumber(3, dec))).to.be.reverted;
  ...
  it('Account3 can transfer after approve', async () => {
    ...
    await token.connect(account2).approve(account3.address, getBNFromNumber(2, dec));
    
    await expect(token.connect(account3).transferFrom(account2.address, account3.address, getBNFromNumber(2, dec))).to.be.not.reverted;
  ...
  it('Account2 is in blacklist and account2 cant approve', async () => {
    ...
    await expect(token.connect(account2).approve(account3.address, getBNFromNumber(2, dec))).to.be.reverted;

    await expect(token.connect(account2).transferFrom(account2.address, account3.address, getBNFromNumber(2, dec))).to.be.reverted;
