...
contract xpndStaking is Ownable, ReentrancyGuard, AccessControl {
     ...
     */
     function setStakingPausedState(bool pausedState)
    ...
    {
        _startStaking(_poolType, _timeAccepting, block.timestamp, 0);
    }
    ...
    function startStakingAllPools() external onlyRole(MANAGER_ROLE) {
        for (uint8 poolType = 0; poolType < poolTypes.length; poolType++) {
            _startStaking(poolType, 0, block.timestamp, 0);
        }
    }
     ...
     */
    function addRewards(uint256 _amount) external notPaused {
     ...
     */
    function stake(uint8 _poolType, uint256 _amount) external nonReentrant notPaused {
     ...
     */
    function withdrawStake(uint16 _stakeId) external nonReentrant notPaused {
    ...
    function withdrawPoolType(uint8 _poolType) external nonReentrant {
        require(_poolType < poolTypes.length, "Invalid poolInstance");
        uint16[] memory poolInstanceArrayByOwner = poolInstancesByOwnerAddress[
            msg.sender
        ];
        require(0 < poolInstanceArrayByOwner.length, "User never staked.");
    ...
    function createPoolInstance(
        ...
        unchecked {
            poolById[poolInstanceCounter].endOfPool =
                uint32(_startTime) +
                uint32(
                    poolTypes[_poolType].periodStaking * MINUTES_IN_DAY * 60
                );
        }

        // we have transferred all accumulated pool 1 rewards to new pool
        defaultRewardBuckets[_poolType] = _initialReward;
    }
     ...
     */
    function computeSettlement(uint256 periodStaking, uint16 stakeId)
        internal
        ...
        if (stakeInfo.settled) {
            ...
            withdrawAmount +=
                (((poolInfo.poolReward * stakeInfo.stakeAmount) /
                    poolInfo.totalStaked) *
                    (poolInfo.endOfPool - stakeInfo.depositTime)) /
                (MINUTES_IN_DAY * 60 * periodStaking);
